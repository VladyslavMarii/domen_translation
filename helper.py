...
try:
    ...
    # pynvml not available: define no-op stubs for GPU monitoring
    def nvmlInit(*args, **kwargs):
        pass
    def nvmlDeviceGetHandleByIndex(*args, **kwargs):
        return None
    def nvmlDeviceGetMemoryInfo(handle=None):
        # Return dummy object with used, free, total attributes
        class _Info:
            used = 0
            free = 0
            total = 0
        return _Info()
    def nvmlShutdown(*args, **kwargs):
        pass
...
def get_random_batch_from_h5(file_path, batch_size, just_one_sample=False):
    """
...
def get_git_commit_id():
    ...
    Returns:
        str: The short commit hash or "unknown" if not in a Git repository
    """
    try:
